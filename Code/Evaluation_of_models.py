...
def get_auc_score(row):
    ...
    row["UZeros_Precision_Score"] = precision_score(y_actuals, [i > 0.10 for i in y_predicts])
    # for uones
    ...
    row["UOnes_Precision_Score"] = precision_score(y_actuals, [i > 0.15 for i in y_predicts])
    return row
